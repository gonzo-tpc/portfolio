...
import { getFunds, getCompanies, getFundMetrics } from '@/lib/db'
...
export default async function Home() {
  ...
  const fundsWithMetrics = await Promise.all(funds.map(async fund => ({
    fund,
    metrics: await getFundMetrics(fund.id),
  })))
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(420px, 1fr))', gap: 16 }}>
        {fundsWithMetrics.map(({ fund, metrics: m }) => (
          <Link key={fund.id} href={'/fund/' + fund.id} style={{ textDecoration: 'none' }}>
            <div style={{ background: '#13131a', border: '1px solid #2a2a3a', borderRadius: 12, padding: 28, cursor: 'pointer' }}>
